...
export async function POST(request: Request) {
  try {
    ...
    // Create pickup using admin client (pickup code will be generated by trigger)
    const { data: pickup, error } = await supabaseAdmin
      ...
      .insert({
        ...
        claimer_id: user.id,
        notes: notes || null,
        status: 'confirmed'  // Set to confirmed immediately to trigger listing status update
      })
      .select()
      .single()
    ...
    // Note: Listing status will be updated automatically by database trigger
